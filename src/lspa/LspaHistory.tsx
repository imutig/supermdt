      ...
      {rows === undefined ? (
        ...
        <div className="overflow-hidden rounded-card border border-border bg-surface">
          {rows.map((r) => {
            ...
            return (
              <div key={r._id} className="flex items-center gap-[10px] border-b border-border pr-[14px] last:border-b-0 hover:bg-surface-2">
                <button
                  onClick={() => navigate(`/lspa/session/${r._id}`)}
                  className="flex min-w-0 flex-1 items-center gap-[13px] px-[16px] py-[13px] text-left"
                >
                ...
                <DeleteButton
                  title="Supprimer la session (elle ne comptera plus)"
                  onDelete={() => toast.guard(deleteSession({ sessionId: r._id as Id<"quizSessions"> }), "Suppression impossible").then((res) => res !== undefined && toast.success("Session supprimée."))}
                />
              </div>
            );
          })}
