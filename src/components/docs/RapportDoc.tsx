...
import { OfficialDoc, DocBlock, Info, buildNarrativeSheets } from "./OfficialDoc";
...
export function RapportDoc({ reportId, body, onClose }: { reportId: Id<"reports">; body: string; onClose: () => void }) {
  ...
  return (
    <OfficialDoc
      ...
      // Feuille 1 : métadonnées et entités ; feuille(s) suivante(s) : le corps
      // rédigé du rapport, paginé au même budget que les autres documents.
      sheets={[
        <>
        ...
        ...buildNarrativeSheets([{ heading: "Détails", text: body.trim() || "Aucun contenu rédigé." }]),
      ]}
      onClose={onClose}
    />
  );
