...
function BotConfigEditor() {
  ...
  const toast = useToast();
  const [f, setF] = useState<null | { presenceChannel: string; dailyChannel: string; rollcallChannel: string; absenceChannel: string; dailyAt: string; rollcallStartAt: string; rollcallEndAt: string; ceremonyAt: string; rollcallPingRole: string; rollcallPingEnabled: boolean; sanctionsChannel: string; sanctionsPingRole: string; ceremonyChannel: string; trackingChannel: string }>(null);
  const shown = f ?? current ?? { presenceChannel: "", dailyChannel: "", rollcallChannel: "", absenceChannel: "", dailyAt: "23:30", rollcallStartAt: "", rollcallEndAt: "", ceremonyAt: "", rollcallPingRole: "", rollcallPingEnabled: false, sanctionsChannel: "", sanctionsPingRole: "", ceremonyChannel: "", trackingChannel: "" };
  ...
  return (
    <div className="rounded-card border border-border bg-surface p-[16px]">
      ...
      <div className="flex flex-col gap-[12px]">
        ...
        <ChanRow label="Salon de suivi des dossiers/contraventions" value={shown.trackingChannel} onChange={set("trackingChannel")} cls={field} hint="Chaque dossier d'arrestation et contravention y est posté en embed, mis à jour en place à chaque changement (chefs, clôture, statut d'amende)." />
